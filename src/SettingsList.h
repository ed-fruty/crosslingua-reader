...
inline SettingInfo buildFontFamilySetting(const SdCardFontRegistry* registry) {
  // Built-in font labels (StrId), index-aligned with CrossPointSettings::FONT_FAMILY
  std::vector<StrId> enumValues = {StrId::STR_EDSLAB, StrId::STR_NOTO_SANS};
  ...
  if (sdFontCount > 0) {
    allStringValues.push_back(I18N.get(StrId::STR_EDSLAB));
  ...
  static const std::vector<SettingInfo> baseList = [] {
    std::vector<SettingInfo> v = {
        ...
        SettingInfo::Enum(StrId::STR_FONT_FAMILY, &CrossPointSettings::fontFamily,
                          {StrId::STR_EDSLAB, StrId::STR_NOTO_SANS}, "fontFamily", StrId::STR_CAT_READER)
            .withTextSettings(),
