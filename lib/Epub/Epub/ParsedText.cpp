...
bool ParsedText::extractNextLine(const GfxRenderer& renderer, const int fontId, const uint16_t width,
  ...
  const bool emitted = extractLine(
      0, 0, pageWidth, wordWidths, wordContinues, wordNoSpaceBefore, lineBreakIndices,
      [&processLine](std::shared_ptr<TextBlock> line, uint32_t) { processLine(std::move(line)); }, renderer, fontId);
...
bool ParsedText::extractLine(const size_t breakIndex, const size_t emittedOrdinal, const int pageWidth,
  ...
  // Only materialize the per-line ruby array for blocks that actually carry ruby; it is moved into
  // the TextBlock and kept for as long as the page is loaded, so allocating lineWordCount empty
  // std::strings for every line of every non-ruby book is pure overhead on a 380 KB no-PSRAM
  // device. An empty vector means "no ruby" to every consumer (hasRuby/getRubyShift/serialize).
